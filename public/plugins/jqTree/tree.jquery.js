...
var ElementsRenderer = (function () {
    ...
    ElementsRenderer.prototype.createNodeLi = function (node, level, is_selected) {
        ...
        li.appendChild(div);
        // title span
        div.appendChild(this.createTitleSpan(node.name, level, is_selected, node.is_open, false, node.widgetData));
        return li;
    };
    ElementsRenderer.prototype.createTitleSpan = function (node_name, level, is_selected, is_open, is_folder, node_widgetData) {
        var title_span = document.createElement("span");
        var classes = "jqtree-title jqtree_common";
        if (is_folder) {
            classes += " jqtree-title-folder";
        } else if(level == 3) {
        	classes = "chartItem jqtree-title jqtree_common";
        }
        title_span.className = classes;
        if(level == 3) {
            title_span.setAttribute("draggable", "true");
            title_span.setAttribute("widget-data", node_widgetData);
        }
