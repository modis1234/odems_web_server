    ...
    w2grid.prototype = {
        ...
        getSearchesHTML: function () {
            ...
            html += '<tr>'+
                    ...
                    '        <div>'+
                    '        <button class="w2ui-btn" onclick="obj = w2ui[\''+ this.name +'\']; if (obj) { obj.searchReset(); }">'+ w2utils.lang('초기화') + '</button>'+
                    '        <button class="w2ui-btn w2ui-btn-blue" onclick="obj = w2ui[\''+ this.name +'\']; if (obj) { obj.search(); }">'+ w2utils.lang('검색') + '</button>'+
                    '        </div>'+
                    '    </td>'+
                    '</tr></tbody></table>';
            return html;
...
//                    break;
        ...
        getCellHTML: function (ind, col_ind, summary, col_span) {
            ...
            // expand icon - 20/04/07 LINE9806: col_ind === 0 --> 3 수정
            if (col_ind === 3 && record && record.w2ui && Array.isArray(record.w2ui.children)) {
                var level  = 0;
